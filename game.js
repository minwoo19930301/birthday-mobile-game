...
const FALLBACK_MAIN = "./assets/portraits/npc-placeholder.svg";
...
function setImageWithFallback(element, preferred, fallback) {
  ...
  let index = 0;
  const baseVisibility = element.style.visibility || "visible";
  ...
  const tryNext = () => {
    if (index >= candidates.length) {
      ...
      element.style.visibility = baseVisibility;
      return;
    }
...
function openDialogue(npcRuntime) {
  ...
  setImageWithFallback(
    dom.portrait,
    [npcRuntime.resolvedPortrait, ...(npcRuntime.def.portraits || [])],
    FALLBACK_MAIN,
  );
...
class MainScene extends Phaser.Scene {
  ...
  preload() {
    ...
    NPC_DEFS.forEach((def) => {
      (def.portraits || []).forEach((file, index) => {
        this.load.image(this.npcPortraitKey(def.id, index), file);
      });
    });
  ...
  npcPortraitKey(id, index) {
    return `npc-portrait-${id}-${index}`;
  }

  pickPortraitAsset(def) {
    const options = def.portraits || [];

    for (let i = 0; i < options.length; i += 1) {
      const key = this.npcPortraitKey(def.id, i);
  ...
  createNpcs() {
    state.npcs = NPC_DEFS.map((def, idx) => {
      ...
      const portraitAsset = this.pickPortraitAsset(def);

      const sprite = this.add.image(anchor.x, anchor.y, portraitAsset.key);
      ...
      const runtime = {
        ...
        sprite,
        badge,
        resolvedPortrait: portraitAsset.src,
      };
      return runtime;
    });
  }
